...
router.get('/:userId', async(req, res) => {
    const borrows = await Borrow.findBorrowsByUserId(req.params.userId)
...
router.put('/', async(req, res) => {
    ...
    let borrows = await Borrow.findBorrowsByUserId(req.body.userId);
    if (!borrows.length) return res.status(403).send('No book borrowed yet.');

    let isBorrowed = borrows.some(borrow => borrow.bookId === req.body.bookId);
    if (!isBorrowed) return res.status(403).send('The user did not borrow the selected book.');
